...
const KEY_BTC = Cypress.env('WITHDRAWAL_BTC_KEY')
const ADDRESS_BTC = Cypress.env('WITHDRAWAL_BTC_ADDRESS')
const AMOUNT_BTC = Cypress.env('WITHDRAWAL_BTC_AMOUNT')
const TOKEN_BTC = 'Bitcoin'

const USERNAME_ETH = Cypress.env('WITHDRAWAL_ETH_USERNAME')
const KEY_ETH = Cypress.env('WITHDRAWAL_ETH_KEY')
const ADDRESS_ETH = Cypress.env('WITHDRAWAL_ETH_ADDRESS')
const AMOUNT_ETH = Cypress.env('WITHDRAWAL_ETH_AMOUNT')
...
if (USERNAME_BTC && KEY_BTC && ADDRESS_BTC && AMOUNT_BTC) {
  describe('Test btc withdrawal', () => {
    ...
    it('Add address to whitelist', () => {
      cy.goToPage('settings', 'Settings')
      cy.wait(5000)
      cy.get('[data-cy=addEthWhiteAddress]').eq(1).click()
      cy.get('[data-cy=addWhiteAddressDialog]').eq(1)
        .find('input')
        .type(ADDRESS_BTC)
        .should('have.value', ADDRESS_BTC)
      
      cy.get('[data-cy=addWhiteAddressDialog]').eq(1)
        .find('.el-button:contains(Add)')
        .click()

      cy.confirm([KEY_BTC])
      cy.contains('Address successfully added', {timeout: 10000}).should('be.visible')
    })
...
if (USERNAME_ETH && KEY_ETH && ADDRESS_ETH && AMOUNT_ETH) {
  describe('Test btc withdrawal', () => {
    ...
    it('Add address to whitelist', () => {
      cy.goToPage('settings', 'Settings')
      cy.wait(5000)
      cy.get('[data-cy=addEthWhiteAddress]').eq(0).click()
      cy.get('[data-cy=addWhiteAddressDialog]').eq(0)
        .find('input')
        .type(ADDRESS_ETH)
        .should('have.value', ADDRESS_ETH)
      
      cy.get('[data-cy=addWhiteAddressDialog]').eq(0)
        .find('.dialog-form_buttons-block > .el-button:contains(Add)')
        .click()
